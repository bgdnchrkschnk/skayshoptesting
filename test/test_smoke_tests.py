...
class SmokeTests:
    ...
    @testcase(url=None, name=None)
    def test_site_is_loaded(self, browser):
    ...
    @testcase(url=None, name=None)
    def test_sales_products_all_items_with_sales_price(self, browser):
    ...
    def test_buy_product(self, browser):
        ...
        assert browser.check_product_in_cart_popup_by_article(
            article="00000037607-013"), "Product has not been found by '00000037607-013' article!"
    ...
    @severity(severity_level.NORMAL)
    @testcase(url=None, name=None)
    def test_site_language(self, browser):
    ...
    @severity(severity_level.CRITICAL)
    @testcase(url=None, name=None)
    def test_register_already_used_email(self, browser):
    ...
    @description(
        "Test provides email in email field of forgot pw module and checks if success pop up was displayed on page")
    @severity(severity_level.CRITICAL)
    @testcase(url=None, name=None)
    def test_forgot_pw_is_correct(self, browser):
        browser = browser.navigate_to_authorization_page()
...
if __name__ == "__main__":
    SmokeTests().test_forgot_pw_is_correct()
    SmokeTests().test_register_already_used_email()
    SmokeTests().test_sales_products_all_items_with_sales_price()
    SmokeTests().test_site_language()
    SmokeTests().test_site_is_loaded()
    SmokeTests().test_buy_product()
